...
CANopenROS2::CANopenROS2() : Node("canopen_ros2")
{
    ...
    RCLCPP_INFO(this->get_logger(), "⚙️ 物理减速比: %.2f, 目标单位/转: %d, 电子齿轮比参数: (%u, %u), 单位/度: %.6f", 
               gear_ratio_, target_units_per_rev_, gear_params.first, gear_params.second, units_per_degree_,
               gear_ratio_, target_units_per_rev_, gear_params.first, gear_params.second, units_per_degree_);
    
    // 调试：打印读取到的原始参数值
    RCLCPP_INFO(this->get_logger(), "🔍 [DEBUG] 节点名称: %s, 读取到的 node_id 参数值: '%s', 减速比: %.2f", 
    ...
    try {
        ...
        RCLCPP_WARN(this->get_logger(), "⚠️ 无法解析 node_id 参数 '%s'，尝试从节点名称提取", node_id_str.c_str(), node_id_str.c_str());
        ...
        if (node_pos != std::string::npos) {
            ...
            if (num_start < node_name.length()) {
                try {
                    ...
                    RCLCPP_INFO(this->get_logger(), "✅ 从节点名称 '%s' 提取到节点 ID: %d", node_name.c_str(), node_id_, node_id_, node_name.c_str());
                } catch (...) {
                    node_id_ = 1;
                    RCLCPP_WARN(this->get_logger(), "⚠️ 无法从节点名称提取节点 ID，使用默认值 1");
                }
            } else {
                node_id_ = 1;
            }
        ...
        if (num_start < node_name.length()) {
            try {
                ...
                if (extracted_id >= 1 && extracted_id <= 3 && extracted_id != node_id_) {
                    RCLCPP_WARN(this->get_logger(), "⚠️ 节点名称 '%s' 指示节点 ID 应为 %d，但参数值为 %d，使用节点名称的值", 
    ...
    RCLCPP_INFO(this->get_logger(), "🏗️ 初始化塔吊控制，节点名称=%s，CAN接口=%s，节点ID=%d (原始参数值: '%s')，减速比=%.2f", 
    ...
    if (can_socket_ < 0)
    {
        RCLCPP_ERROR(this->get_logger(), "❌ CAN 套接字初始化失败，无法继续");
        return;
    }
