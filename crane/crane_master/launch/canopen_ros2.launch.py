...
def launch_nodes(context):
    ...
    crane_params_yaml = os.path.join(
        get_package_share_directory('crane_master'),
        'config',
        'crane_params.yaml'
    )
    ...
    # Node 2 (Trolley): namespace=trolley, node_id=2
    node2_config = {
        'package':    'crane_master',
        'executable': 'tower_crane',
    ...
    # Node 3 (Slewing): namespace=slewing, node_id=3
    node3_config = {
        'package':    'crane_master',
        'executable': 'tower_crane',
    ...
    sync_trajectory_server = Node(
        ...
        output='screen',
        emulate_tty=True,
        parameters=[{'can_interface': can_interface_value}]
    )

    return [
        LogInfo(msg=f"🏗️ Starting CANopen ROS2 nodes (node_id={node_id_value})..."),
        LogInfo(msg=f"📄 Loading parameters from: {crane_params_yaml}"),
        check_can,
        *nodes_to_launch,
