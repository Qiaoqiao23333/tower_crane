    ...
    for axis_key, meta in axis_defs.items():
        ...
        all_configs[nid] = {
            ...
            'parameters': [{
                ...
                'homing_method':                  int(merged.get('homing_method', 35)),
                'homing_fast_velocity':           float(merged.get('homing_fast_velocity', 10.0)),
                'homing_slow_velocity':           float(merged.get('homing_slow_velocity', 2.0)),
                'homing_acceleration':            float(merged.get('homing_acceleration', 10.0)),
                'homing_timeout_s':               float(merged.get('homing_timeout_s', 30.0)),
                'homing_store_parameters':        bool(merged.get('homing_store_parameters', False)),
                'homing_restore_previous_mode':   bool(merged.get('homing_restore_previous_mode', True)),
            }]
        }
